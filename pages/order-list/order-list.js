...
Page({
  ...
  orderDetail: function (e) {
    ...
    wx.navigateTo({
      ...
      //  "/pages/order-detail/order-detail?id=" + orderId + '&share=1'
    })
  },
    ...
    wx.showModal({
      ...
      success: function (res) {
        if (res.confirm) {
          ...
          wx.request({
            url:"1",
            //  app.siteInfo.url + app.siteInfo.subDomain + '/order/close',
            data: {
  ...
  toPayTap: function (e) {
    ...
    wx.request({
      url:"http://jd.2004.com/",
      //  app.siteInfo.url + app.siteInfo.subDomain + '/user/amount',
      data: {
        token: app.globalData.token
      },
      success: function (res) {
        if (res.data.code == 0) {
          ...
          if (money <= 0) {
            ...
            wx.request({
              url:"http://jd.2004.com/",
              //  app.siteInfo.url + app.siteInfo.subDomain + '/order/pay',
              method: 'POST',
              header: {
  ...
  getOrderStatistics: function () {
    ...
    wx.request({
      url:"http://jd.2004.com/",
      //  app.siteInfo.url + app.siteInfo.subDomain + '/order/statistics',
	  ...
	  wx.showModal({
	      ...
	      success: function(res) {
	        if (res.confirm) {
	          ...
	          wx.request({
              url:"http://jd.2004.com/",
              //  app.globalData.urls + '/order/delivery',
	            data: {
	              token: app.globalData.token,
	              orderId: orderId
	            },
  ...
  onShow: function (e) {
    ...
    wx.request({
      url: "http://jd.2004.com/",
      // app.siteInfo.url + app.siteInfo.subDomain + '/order/list',
      data: postData,
      success: (res) => {
				console.log(res)
        wx.hideLoading();
